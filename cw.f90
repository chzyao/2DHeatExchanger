...
!coefficient for RK sub-time steps
  ...
  do j=1,ny
     do i=1,nx
        rho(i,j)=rho(i,j)+coef(1,k)*dlt*fro(i,j)+coef(2,k)*dlt*gro(i,j)
        gro(i,j)=fro(i,j)
        
        rou(i,j)=rou(i,j)+coef(1,k)*dlt*fru(i,j)+coef(2,k)*dlt*gru(i,j)
        gru(i,j)=fru(i,j)
        
        rov(i,j)=rov(i,j)+coef(1,k)*dlt*frv(i,j)+coef(2,k)*dlt*grv(i,j)
        grv(i,j)=frv(i,j)
        
        roe(i,j)=roe(i,j)+coef(1,k)*dlt*fre(i,j)+coef(2,k)*dlt*gre(i,j)
        gre(i,j)=fre(i,j)
        
        scp(i,j)=scp(i,j)+coef(1,k)*dlt*ftp(i,j)+coef(2,k)*dlt*gtp(i,j)
        gtp(i,j)=ftp(i,j)
     enddo
  enddo
